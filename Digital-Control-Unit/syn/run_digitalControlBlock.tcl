################################################################################
# Filename: run_digitalControlBlock.tcl
# Author: XIE BAOHUI
...
analyze -format verilog {../rtl/digitalControlBlock.v}
elaborate digitalControlBlock; # top module name

# store the unmapped results
write -hierarchy -format ddc -output results/digitalControlBlock.unmapped.ddc
...
source digitalControlBlock.constraints.tcl
...
check_design > reports/digitalControlBlock.check_design.rpt; # dump to the file
...
# Write out design
write -format verilog -hierarchy -output results/digitalControlBlock.mapped.v
write -format ddc -hierarchy -output results/digitalControlBlock.mapped.ddc
write_sdf results/digitalControlBlock.mapped.sdf
write_sdc -nosplit results/digitalControlBlock.mapped.sdc

# Generate reports
report_qor > reports/digitalControlBlock.mapped.qor.rpt
report_timing -transition_time -nets -attribute -nosplit \
  > reports/digitalControlBlock.mapped.timing.rpt
report_area -nosplit > reports/digitalControlBlock.mapped.area.rpt
report_power -nosplit > reports/digitalControlBlock.mapped.power.rpt
