...
# - link_library
################################################################################
set_app_var search_path ". /afs/ee.ust.hk/staff/ee/dept/public/eesm6980/digital/GSCLib3/src ../rtl $search_path"
set_app_var target_library "GSCLib.db"
...
analyze -format verilog {manchester_dec.v}
elaborate manchester_dec; # top module name

# store the unmapped results
write -hierarchy -format ddc -output results/manchester_dec.unmapped.ddc
...
source manchester_dec.constraints.tcl
...
check_design > reports/manchester_dec.check_design.rpt; # dump to the file
...
# Write out design
write -format verilog -hierarchy -output results/manchester_dec.mapped.v
write -format ddc -hierarchy -output results/manchester_dec.mapped.ddc
write_sdf results/manchester_dec.mapped.sdf
write_sdc -nosplit results/manchester_dec.mapped.sdc

# Generate reports
report_qor > reports/manchester_dec.mapped.qor.rpt
report_timing -transition_time -nets -attribute -nosplit \
  > reports/manchester_dec.mapped.timing.rpt
report_area -nosplit > reports/manchester_dec.mapped.area.rpt
report_power -nosplit > reports/manchester_dec.mapped.power.rpt
