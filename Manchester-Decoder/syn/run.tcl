...
analyze -format verilog {../rtl/decoder.v}
elaborate decoder; # top module name

# store the unmapped results
write -hierarchy -format ddc -output results/decoder.unmapped.ddc
...
source decoder.constraints.tcl
...
check_design > reports/decoder.check_design.rpt; # dump to the file
...
# Write out design
write -format verilog -hierarchy -output results/decoder.mapped.v
write -format ddc -hierarchy -output results/decoder.mapped.ddc
write_sdf results/decoder.mapped.sdf
write_sdc -nosplit results/decoder.mapped.sdc

# Generate reports
report_qor > reports/decoder.mapped.qor.rpt
report_timing -transition_time -nets -attribute -nosplit \
  > reports/decoder.mapped.timing.rpt
report_area -nosplit > reports/decoder.mapped.area.rpt
report_power -nosplit > reports/decoder.mapped.power.rpt
